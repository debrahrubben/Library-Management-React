...
const AddBookForm = () => {
  ...
  const handleAddBook = () => {
    axios.post('http://localhost:3001/books', { borrower: borrower, mobile_number: mobileNumber, book_title: book })
      .then(response => {
          ...
          <div className="book-cards-container">
            {borrowedBooks.map(borrowedBook => (
              <div key={borrowedBook.id} className="book-card">
                <div>
                  <span style={{ color: '#4f2222', fontWeight: 'bold' }}>Person: </span>
                  <span style={{ textTransform: 'uppercase' }}>{borrowedBook.borrower}</span>
                </div>
                <div>
                  <span style={{ color: '#4f2222', fontWeight: 'bold' }}>Mobile Number: </span>
                  <span>{borrowedBook.mobile_number}</span> {/* Display the mobile number */}
                </div>
                <div>
                  <span style={{ color: '#4f2222', fontWeight: 'bold' }}>Book: </span>
                  <span style={{ textTransform: 'uppercase' }}>{borrowedBook.book_title}</span>
                  <Button onClick={() => handleDeleteBook(borrowedBook.id)} style={{backgroundColor:'#5F8670',}}>Returned<CheckCircleOutlined /></Button>
                </div>
              </div>
            ))}
          </div>
